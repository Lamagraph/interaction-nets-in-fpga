...
- [Project overview](#project-overview)
  - [interaction-nets-in-fpga.cabal](#namecabal)
...
│   └── unittests.hs
├── cabal.project
├── interaction-nets-in-fpga.cabal
└── stack.yaml
```
...
Note: If you need a newer Clash version, simply change the version bounds in `interaction-nets-in-fpga.cabal` and follow the hints given by Stack.

## src/
This is where the source code of the project lives, as specified in `interaction-nets-in-fpga.cabal`. It contains a single file, `Example/Project.hs` which starts with:
...
# Change the license
By default `interaction-nets-in-fpga.cabal` sets its `license` field to `BSD-2-Clause`. You might want to change this.
